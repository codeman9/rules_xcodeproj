...
def xcodeproj(
    ...
    **EXAMPLE**
    ...
    Args:
        ...
        enable_previews: Optional. Whether to enable Xcode 16+ SwiftUI Preview
            support.

            When enabled, additional Swift compiler flags
            (`-enable-implicit-dynamic`, `-enable-private-imports`,
            `-enable-dynamic-replacement-chaining`) and build settings
            (`ENABLE_DEBUG_DYLIB`, `ENABLE_XOJIT_PREVIEWS`) are added to make
            previews work in Build-with-Bazel mode. These flags have a minor
            performance cost, so only enable this if you need previews.
        import_index_build_indexstores: Optional. Whether to import the index
            stores generated by Index Build.

            This is useful if you want to use the index stores generated by
